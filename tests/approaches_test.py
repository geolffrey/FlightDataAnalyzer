...
class TestApproachInformation(unittest.TestCase):
    ...
    def test_can_operate(self):
        ...
        self.assertTrue(ApproachInformation.can_operate(
            ('Approach And Landing', 'Altitude AAL', 'Latitude Prepared', 'Longitude Prepared',
             'Latitude (Coarse)', 'Longitude (Coarse)', 'Takeoff', 'Touchdown'),
            ac_type=aeroplane))
...
class TestPalmBeach(unittest.TestCase):
    # Flight fwd:fa0e589f0533
    # Non-precise aircraft. Touchdown is between 10L and 10R,
    # although the final path is nicely established on 10L
    @patch('analysis_engine.approaches.api')
    def test_final_aligned_with_rwy_but_tdwn_offset(self, api):

        get_handler = Mock()
        get_handler.get_nearest_airport.return_value = [airports['palm beach']]
        api.get_handler.return_value = get_handler
        ...
        def fetch(par_name):
            try:
                ...
                return None
        root = os.path.join(approaches_path, 'Landing_test_fwd_fa0e589f0533_')
        app_start = 5026.0
        app_end = 5387.0
        ...
        approaches.derive(fetch('Altitude AAL'),
                          ...
                          fetch('Latitude'),
                          fetch('Longitude'),
                          fetch('Latitude (Coarse)'),
                          fetch('Longitude (Coarse)'),
                          fetch('ILS Localizer'),
                          fetch('ILS Glideslope'),
                          fetch('ILS Frequency'),
                          A(name='AFR Landing Airport', value=None),
                          A(name='AFR Landing Runway', value=None) ,
                          KPV('Latitude At Touchdown', items=[
                              KeyPointValue(index=5302.09375, value=26.681671142578125)
                          ]),
                          KPV('Longitude At Touchdown', items=[
                              KeyPointValue(index=5302.09375, value=-80.1024878025055)
                          ]),
                          A('Precise Positioning', False))
        self.assertEqual(approaches[0].airport['name'], 'Palm Beach Intl')
        self.assertEqual(approaches[0].landing_runway['identifier'], '10L')
